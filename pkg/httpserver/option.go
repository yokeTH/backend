...
// WithOAPIProtection set the swagger basic auth username and password
func WithOAPIProtection(username, password string) ServerOption {
	return func(s *Server) {
		s.config.OAPIUser = username
		s.config.OAPIPass = password
	}
}
